...
import { createStripeCurrencyCheckoutSession, isStripeConfigured, verifyStripeWebhookEvent, } from "./payments.js";
const app = express();
const port = Number(process.env.PORT || 4000);
const store = await createStore();
const FEATURED_FALLBACK_MAPS = {
    "skyrail-sprint": {
        spawn: { x: 0, y: 2, z: 0 },
        checkpoints: [
...
function getFeaturedFallbackDetail(slug) {
...
app.post("/payments/stripe/webhook", express.raw({ type: "application/json" }), (req, res) => {
    ...
    try {
        ...
        if (event.type === "checkout.session.completed") {
            ...
            if (orderId) {
                void store.fulfillCurrencyPurchaseOrder(orderId, session.id).catch((error) => {
                    sendAlert({
                        severity: "error",
...
function enforceEconomyRateLimit(req, action) {
...
function extractIdempotencyKey(req) {
    ...
    const bodyValue = req.body && typeof req.body === "object" ? req.body.requestId : undefined;
...
function statusForEconomyError(error) {
...
app.get("/health", (_req, res) => {
    const response = { ok: true, service: "fairblox-api" };
    res.json(response);
});
...
app.put("/me/account-state", (req, res) => {
    void store
        .saveAccountState(String(req.header("x-session-token") || ""), req.body)
        .then((state) => {
        const response = { state };
        res.json(response);
    })
...
app.post("/marketplace/items", (req, res) => {
    void store
        .createMarketplaceItem(String(req.header("x-session-token") || ""), req.body)
        .then((item) => {
...
app.post("/marketplace/trades", (req, res) => {
    ...
    void store
        .tradeMarketplaceLimited(String(req.header("x-session-token") || ""), payload)
        .then((state) => {
        const response = { state };
        res.json(response);
    })
...
app.post("/marketplace/purchase", (req, res) => {
    ...
    void store
        .purchaseMarketplaceItem(String(req.header("x-session-token") || ""), payload)
        .then((state) => {
        const response = { state };
        res.json(response);
    })
...
app.post("/economy/purchase-currency", (req, res) => {
    ...
    void store
        .purchaseCurrency(String(req.header("x-session-token") || ""), payload)
        .then((state) => {
        const response = { state };
        res.json(response);
    })
...
app.post("/payments/currency/checkout", (req, res) => {
    ...
    void store
        .createCurrencyPurchaseOrder(token, req.body)
        .then(async (order) => {
        ...
        const updatedOrder = await store.attachCurrencyPurchaseOrderCheckout(order.id, checkout.sessionId, checkout.checkoutUrl);
        const response = { order: updatedOrder };
        res.status(201).json(response);
    })
        .catch((error) => {
...
app.post("/games", (req, res) => {
    void store
        .createGameDraft(String(req.header("x-session-token") || ""), req.body)
        .then((game) => {
...
app.put("/me/games/:id/map", (req, res) => {
    void store
        .saveGameMap(String(req.header("x-session-token") || ""), req.params.id, req.body)
        .then((game) => {
        res.json({ game });
    })
        .catch((error) => {
...
app.post("/me/games/:id/publish", (req, res) => {
    void store
        .publishGame(String(req.header("x-session-token") || ""), req.params.id, req.body)
        .then((game) => {
        res.json({ game });
    })
        .catch((error) => {
